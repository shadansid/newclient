  ...
  return (
    ...
    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', width:'50%' , padding:'20px'}}>
        ...
        <div>{ele.currency}</div>
            ...
            <div>{ ele.amount}</div>
            <div>{ ele.dt}</div>
        <IconButton sx={{fontSize:'12px',color:'grey'}}>{ele.status}&nbsp;{ele.status === 'Pending'?<Pending  sx={{color:'orange',fontSize:'16px' }}/>:<Success  sx={{color:'green',fontSize:'16px' }}/>}</IconButton>
