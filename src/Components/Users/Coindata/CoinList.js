...
const showcoin = ()=>{
  ...
  context.dispatcher({type:"update",payload:{symbol :props.symbol, custom:props.custom, Cprice:props.Cprice,price:props.price, DisbableBtn:props.DisbableBtn, boolVal:boolVal} })
...
const [boolVal, setboolVal] = useState(0)
 ...
 {<Box  className="coinlis" style={{ borderTop: (props.bg == '#FFFFFF'?'1px solid #E7E7E8':'1px solid #363c4e'), display:'flex' , alignItems:'center', justifyContent:'space-around'}} id={props.symbol}>
    ...
    cursor: 'pointer'}} onClick={showcoin} >
...
<div id="price" style={{width:'40%',color:((props.color=="g")?"#089981":(props.color=="r")?"#F23645":"")}}>${props.Cprice===null?props.price:props.Cprice}</div>
        ...
        <Box sx={{ display: { xs: 'block', sm: 'none' }, width: '40%' }} id="symbol">
          
          
        
          <Link style={{ textDecoration: 'none' }}><IconButton sx={{ color: 'grey', fontSize: '0.8rem', padding: '6px' }}> <Button onClick={() => {
            sendValDs();
           
    showcoin();

            // props.disbableBtn();
          }}   variant='contained' size='small' sx={{ backgroundColor: '#363BE6', fontSize: '11px' }}>Trade</Button></IconButton></Link>
